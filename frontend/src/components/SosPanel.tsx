...
export function SosPanel({ open, onClose, origin, destination, route }: { open: boolean; onClose: () => void; origin: string; destination: string; route?: RouteData }) {
  ...
  return <div role="dialog" aria-modal="true" aria-label="Emergency and share safe trip" className="fixed inset-0 z-50 bg-slate-950/45 backdrop-blur-sm flex items-center justify-center p-4">
    <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-lg overflow-hidden">
      <div className="p-4 bg-rose-50 border-b border-rose-200 flex justify-between"><div className="flex gap-2"><Siren className="w-5 h-5 text-rose-600"/><div><h2 className="font-bold text-slate-900">Emergency / Share Safe Trip</h2><p className="text-xs text-slate-600">A demo check-in, not an emergency dispatch service</p></div></div><button onClick={onClose} aria-label="Close share panel"><X className="w-5 h-5"/></button></div>
      <div className="p-4 space-y-3">
        ...
        <button disabled={!route} onClick={copy} className="w-full rounded-xl bg-emerald-600 text-white py-2.5 text-xs font-bold flex items-center justify-center gap-2 disabled:opacity-50">{copied ? <Check className="w-4 h-4"/> : <Copy className="w-4 h-4"/>}{copied ? 'Copied' : 'Copy trip message'}</button>
